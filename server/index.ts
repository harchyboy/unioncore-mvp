...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    const uploadDir = path.resolve(__dirname, "..", "client", "uploads", "properties");
...
async function startServer() {
  ...
  app.delete('/api/properties/delete-image/:filename', (req, res) => {
    try {
      ...
      const filePath = path.resolve(__dirname, "..", "client", "uploads", "properties", filename);
  ...
  server.listen(port, "0.0.0.0", () => {
    ...
    console.log(`Image uploads will be stored in: ${path.resolve(__dirname, "..", "client", "uploads", "properties")}`);
  });
}
