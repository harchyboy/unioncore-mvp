...
function populateHeader() {
  ...
  // Update header
  document.getElementById("property-detail-name").textContent =
    currentProperty.name;
  document.getElementById("property-detail-address").textContent =
    currentProperty.address;
  document.getElementById("property-detail-sector").textContent =
    currentProperty.type;
  document.getElementById("property-detail-size").textContent =
    currentProperty.totalArea.toLocaleString();
  document.getElementById("property-detail-rent").textContent =
  ...
  // Update breadcrumb
  document.getElementById("property-detail-breadcrumb").textContent =
    currentProperty.name;
}
...
function populateMetrics() {
  ...
  document.getElementById("property-detail-id").textContent =
    currentProperty.id;
  document.getElementById("property-detail-occupancy").textContent =
    currentProperty.occupancyRate;
  document.getElementById("property-detail-revenue").textContent =
    currentProperty.annualRevenue.toLocaleString();
  document.getElementById("property-detail-units-count").textContent =
...
function populateOverviewTab() {
  ...
  gallery.innerHTML = currentProperty.images
    .map(
      (img, index) => `
  ...
  // Description
  document.getElementById("property-detail-description").textContent =
    currentProperty.description;

  // Specifications
  document.getElementById("property-spec-address").textContent =
    currentProperty.address;
  document.getElementById("property-spec-type").textContent =
    currentProperty.type;
  document.getElementById("property-spec-area").textContent =
    `${currentProperty.totalArea.toLocaleString()} sqft`;
  document.getElementById("property-spec-floors").textContent =
    currentProperty.floors;
  document.getElementById("property-spec-built").textContent =
    currentProperty.builtYear;
  document.getElementById("property-spec-location").textContent =
    currentProperty.location;

  // Amenities
  const amenitiesList = document.getElementById("property-amenities-list");
  amenitiesList.innerHTML = currentProperty.amenities
    .map(
      amenity => `
  ...
  // Map address
  document.getElementById("property-map-address").textContent =
    currentProperty.address;
}
...
function populateUnitsTab() {
  ...
  // Occupancy summary
  document.getElementById("units-occupied-count").textContent =
    currentProperty.occupiedUnits;
  document.getElementById("units-total-count").textContent =
    currentProperty.totalUnits;
  document.getElementById("units-occupancy-rate").textContent =
    currentProperty.occupancyRate;

  // Units table
  const tableBody = document.getElementById("units-table-body");
  tableBody.innerHTML = currentProperty.units
    .map(unit => {
      const statusColors = {
...
function populateAvailabilityTab() {
  ...
  const upcomingUnits = currentProperty.units.filter(
    u => u.status === "Occupied" && u.leaseEnd
  );

  if (upcomingUnits.length === 0) {
    upcomingList.innerHTML =
      '<p class="text-sm text-concrete">No upcoming availability</p>';
  } else {
    upcomingList.innerHTML = upcomingUnits
      .slice(0, 5)
      .map(
        unit => `
...
function populatePerformanceTab() {
  ...
  // KPIs
  document.getElementById("perf-total-revenue").textContent =
    currentProperty.annualRevenue.toLocaleString();
  document.getElementById("perf-occupancy-rate").textContent =
    currentProperty.occupancyRate;
  document.getElementById("perf-avg-rent").textContent =
    currentProperty.pricePerSqFt;
...
function populateDocumentsTab() {
  ...
  if (!currentProperty.documents || currentProperty.documents.length === 0) {
    documentsList.innerHTML =
      '<p class="text-sm text-concrete">No documents available</p>';
    return;
  }
  ...
  documentsList.innerHTML = currentProperty.documents
    .map(
      doc => `
...
function populateMarketingTab() {
  ...
  if (currentProperty.marketing && currentProperty.marketing.platforms) {
    platformsList.innerHTML = currentProperty.marketing.platforms
      .map(
        platform => `
  ...
  // Preview
  document.getElementById("marketing-preview-image").src =
    currentProperty.images[0];
  document.getElementById("marketing-preview-name").textContent =
    currentProperty.name;
  document.getElementById("marketing-preview-address").textContent =
    currentProperty.address;
  document.getElementById("marketing-preview-description").textContent =
    currentProperty.description.substring(0, 100) + "...";
  document.getElementById("marketing-preview-size").textContent =
    currentProperty.totalArea.toLocaleString();
  document.getElementById("marketing-preview-price").textContent =
    currentProperty.pricePerSqFt;

  // Performance metrics
  if (currentProperty.marketing) {
    document.getElementById("marketing-views").textContent =
      currentProperty.marketing.views || 0;
    document.getElementById("marketing-enquiries").textContent =
      currentProperty.marketing.enquiries || 0;
    const conversionRate =
      currentProperty.marketing.enquiries && currentProperty.marketing.views
        ? (
            (currentProperty.marketing.enquiries /
              currentProperty.marketing.views) *
            100
          ).toFixed(1)
        : 0;
    document.getElementById("marketing-conversion").textContent =
      conversionRate;
  }
}
