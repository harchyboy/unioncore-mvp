...
function filterTickets() {
  ...
  if (searchInput)
    currentTicketFilters.search = searchInput.value.toLowerCase();
  ...
  filteredTickets = allTicketsData.filter(ticket => {
    ...
    if (
      ...
      ticket.status !== currentTicketFilters.status
    ) {
      return false;
    }
    ...
    // Priority filter
    if (
      currentTicketFilters.priority !== "all" &&
      ticket.priority !== currentTicketFilters.priority
    ) {
      return false;
    }
    ...
    // Category filter
    if (
      currentTicketFilters.category !== "all" &&
      ticket.category !== currentTicketFilters.category
    ) {
      return false;
    }
    ...
    // Assigned filter
    if (
      currentTicketFilters.assigned !== "all" &&
      ticket.assignedTo !== currentTicketFilters.assigned
    ) {
      return false;
    }
...
function populateTicketDetail(ticket) {
  ...
  document.getElementById("ticketDetailDescription").textContent =
    ticket.description;

  // Reporter info
  document.getElementById("ticketDetailReporterName").textContent =
    ticket.reporter.name;
  document.getElementById("ticketDetailReporterCompany").textContent =
    ticket.reporter.company;
  document.getElementById("ticketDetailReporterEmail").textContent =
    ticket.reporter.email;
  document.getElementById("ticketDetailReporterPhone").textContent =
  ...
  document.getElementById("ticketDetailAssigned").textContent =
    ticket.assignedTo;
  document.getElementById("ticketDetailAssignedAvatar").textContent =
    ticket.assignedTo.charAt(0);
  document.getElementById("ticketDetailCreated").textContent = formatDate(
    ticket.created
  );
  document.getElementById("ticketDetailUpdated").textContent = formatDate(
    ticket.lastUpdated
  );

  // SLA deadline
  if (ticket.slaDeadline && ticket.status !== "Resolved") {
    document.getElementById("ticketDetailSLA").textContent = formatDate(
      ticket.slaDeadline
    );
    document
      .getElementById("ticketDetailSLASection")
  ...
  if (ticket.attachments && ticket.attachments.length > 0) {
    attachmentsContainer.innerHTML = ticket.attachments
      .map(
        att => `
      ...
      .join("");
    document
      .getElementById("ticketDetailAttachmentsSection")
      .classList.remove("hidden");
  } else {
    document
      .getElementById("ticketDetailAttachmentsSection")
...
function populateTimeline(ticket) {
  ...
  timelineContainer.innerHTML = ticket.timeline
    .map((entry, index) => {
...
function populateCommunications(ticket) {
  ...
  commsContainer.innerHTML = ticket.communications
    .map(comm => {
...
function populateResolution(ticket) {
  ...
  if (ticket.status === "Resolved" && ticket.resolvedDate) {
    ...
    document.getElementById("ticketResolvedDate").textContent = formatDate(
      ticket.resolvedDate
    );
    document.getElementById("ticketResolvedBy").textContent = ticket.resolvedBy;
    document.getElementById("ticketResolutionTime").textContent =
      `${ticket.resolutionTime} days`;
    ...
    document.getElementById("ticketTenantFeedback").textContent =
      ticket.tenantFeedback || "No feedback provided";
    document.getElementById("ticketResolutionNotes").textContent =
      ticket.resolutionNotes || "No notes provided";
    document.getElementById("ticketRootCause").textContent =
    ...
    if (ticket.actionsTaken && ticket.actionsTaken.length > 0) {
      actionsContainer.innerHTML = ticket.actionsTaken
        .map(
          action => `
...
function updateTicketStatus(newStatus) {
  ...
  if (newStatus === "Resolved") {
    ...
    currentTicket.resolutionTime = (timeDiff / (1000 * 60 * 60 * 24)).toFixed(
      1
    );
  }
