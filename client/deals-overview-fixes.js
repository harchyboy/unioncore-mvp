...
(function () {
  ...
  const SAMPLE_DEALS = [
    {
      ...
      probability: 85,
      risk: "Medium",
      nextMilestone: "Contract Review",
      dueDate: "Nov 15",
      owner: "Sarah Chen",
      ownerAvatar:
        "https://storage.googleapis.com/uxpilot-auth.appspot.com/avatars/avatar-1.jpg",
      landlord: "Canary Wharf Group",
      tenant: "Tech Corp Ltd",
      daysInStage: 12,
    ...
    {
      id: "DL_SH_2024_002",
      name: "Shoreditch Retail Space",
      property: "156 Brick Lane",
      propertyType: "Retail",
      sqft: "3,500",
      stage: "Contract",
      value: 850000,
      probability: 92,
      risk: "Low",
      nextMilestone: "Final Signatures",
      dueDate: "Nov 20",
      owner: "Mike Rodriguez",
      ownerAvatar:
        "https://storage.googleapis.com/uxpilot-auth.appspot.com/avatars/avatar-3.jpg",
      landlord: "British Land",
    ...
    {
      id: "DL_SB_2024_004",
      name: "Southbank Office",
      property: "Belvedere Road",
      propertyType: "Office",
      sqft: "12,000",
      stage: "Live",
      value: 1800000,
  ...
  function enhanceFilterPanel() {
    const filterButton = document.querySelector(
      '[data-filter-toggle="filter-panel"]'
    );
    ...
    const tableContainer = document.querySelector(
      "#active-deals-section .flex.items-center.justify-between"
    );
    if (tableContainer) {
      tableContainer.parentElement.insertAdjacentHTML(
        "afterend",
        filterPanelHTML
      );
    ...
    if (sectionTitle) {
      const statusLabel =
        currentFilters.status === "active"
          ? "Active Deals"
          : currentFilters.status === "closed-won"
            ? "Closed/Won Deals"
            : currentFilters.status === "closed-lost"
              ? "Closed/Lost Deals"
              : "All Deals";
  ...
  function createDealRow(deal) {
    ...
    const velocityIcon =
      deal.velocity === "fast" ? "🟢" : deal.velocity === "stuck" ? "🔴" : "🟡";
    const velocityColor =
      deal.velocity === "fast"
        ? "text-green-600"
        : deal.velocity === "stuck"
  ...
  window.applyDealsFilters = function () {
    currentFilters.landlords = Array.from(
      document.getElementById("filter-landlord").selectedOptions
    ).map(o => o.value);
    currentFilters.tenants = Array.from(
      document.getElementById("filter-tenant").selectedOptions
    ).map(o => o.value);
    currentFilters.stages = Array.from(
      document.getElementById("filter-stage").selectedOptions
    ).map(o => o.value);
    currentFilters.propertyTypes = Array.from(
      document.getElementById("filter-property-type").selectedOptions
    ).map(o => o.value);
    currentFilters.riskLevels = Array.from(
      document.getElementById("filter-risk").selectedOptions
    ).map(o => o.value);
