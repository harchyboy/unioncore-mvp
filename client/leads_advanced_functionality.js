...
function searchLeads() {
  ...
  leadCards.forEach(card => {
    ...
    const industry = card
      .querySelector(".flex.items-center.space-x-4 span")
...
function bulkDisqualify() {
  ...
  if (
    confirm(`Are you sure you want to disqualify ${selectedLeads.size} leads?`)
  ) {
...
function exportLeads(selectedOnly = false) {
  ...
  // Create CSV content
  let csv =
    "Lead ID,Company,Contact,Grade,BANT Score,Budget,Authority,Need,Timeline,Industry,Location,Desks,Date Added\n";
  ...
  leadCards.forEach(card => {
    ...
    const contact = card
      ...
      .textContent.replace("Contact: ", "");
    const score = card.querySelector(".text-2xl.font-bold").textContent;

    const bantScores = card.querySelectorAll(
    ...
    const industry =
      details[0]?.textContent.trim().replace(/\s+/g, " ") || "N/A";
    const location =
      details[1]?.textContent.trim().replace(/\s+/g, " ") || "N/A";
    const desks = details[2]?.textContent.trim().replace(/\s+/g, " ") || "N/A";
    const dateAdded =
      details[3]?.textContent.trim().replace(/\s+/g, " ") || "N/A";
...
function confirmAssignment() {
  ...
  const priority =
    document.querySelector('input[name="assignPriority"]:checked')?.value ||
    "medium";
...
function showScheduleModal(leadId = null) {
  ...
  if (leadId) {
    ...
    if (leadCard) {
      ...
      document.getElementById("scheduleLeadGrade").textContent =
        `Grade ${grade} Lead`;
    }
  }
...
function confirmSchedule() {
  ...
  const propertyName = document.getElementById("schedulePropertySelect")
    .options[document.getElementById("schedulePropertySelect").selectedIndex]
    .text;

  showToast(
    `Viewing scheduled for ${propertyName} on ${date} at ${time}`,
    "success"
  );
  closeScheduleModal();
