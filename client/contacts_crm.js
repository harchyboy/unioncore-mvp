...
function populateCompanyFilters() {
  ...
  const addContactCompanySelect = document.querySelector(
    '#addContactForm select[name="companyId"]'
  );
...
function filterContacts() {
  ...
  filteredContacts = contactsData.filter(contact => {
    ...
    // Tags filter
    const matchesTags =
      tagsFilter === "all" || contact.tags.includes(tagsFilter);
...
function sortContacts() {
  ...
  filteredContacts.sort((a, b) => {
    switch (sortBy) {
      ...
      case "company":
        return getCompanyName(a.companyId).localeCompare(
...
function switchContactView(view) {
  ...
  document
    ...
    .classList.toggle("bg-white", view === "grid");
  document
    .getElementById("gridViewBtn")
    .classList.toggle("shadow-sm", view === "grid");
  document
    .getElementById("gridViewBtn")
    .classList.toggle("text-stone-700", view === "grid");
  document
    .getElementById("gridViewBtn")
    .classList.toggle("text-stone-600", view !== "grid");

  document
    .getElementById("listViewBtn")
    .classList.toggle("bg-white", view === "list");
  document
    .getElementById("listViewBtn")
    .classList.toggle("shadow-sm", view === "list");
  document
    .getElementById("listViewBtn")
    .classList.toggle("text-stone-700", view === "list");
  document
    .getElementById("listViewBtn")
    .classList.toggle("text-stone-600", view !== "list");

  document
    .getElementById("tableViewBtn")
    .classList.toggle("bg-white", view === "table");
  document
    .getElementById("tableViewBtn")
    .classList.toggle("shadow-sm", view === "table");
  document
    .getElementById("tableViewBtn")
    .classList.toggle("text-stone-700", view === "table");
  document
    .getElementById("tableViewBtn")
    .classList.toggle("text-stone-600", view !== "table");

  // Show/hide views
  document
    .getElementById("contactsGridView")
    .classList.toggle("hidden", view !== "grid");
  document
    .getElementById("contactsListView")
    .classList.toggle("hidden", view !== "list");
  document
    .getElementById("contactsTableView")
...
function renderGridView() {
  ...
  filteredContacts.forEach(contact => {
    ...
    card.className =
      "bg-white rounded-xl shadow-sm p-6 border border-stone-200 hover:shadow-md transition-shadow cursor-pointer";
...
function renderListView() {
  ...
  filteredContacts.forEach(contact => {
    ...
    const card = document.createElement("div");
    card.className =
      "bg-white rounded-xl shadow-sm p-4 border border-stone-200 hover:shadow-md transition-shadow cursor-pointer";
    card.onclick = () => openContactDetail(contact.id);
...
function updateContactKPIs() {
  ...
  const activeTenants = filteredContacts.filter(
    c => c.type === "Tenant"
  ).length;
...
function openContactDetail(contactId) {
  ...
  // Set header
  document.getElementById("contactDetailAvatar").className =
    `w-16 h-16 rounded-full ${avatarColor} flex items-center justify-center text-white text-2xl font-bold`;
  document.getElementById("contactDetailAvatar").textContent = initials;
  document.getElementById("contactDetailName").textContent =
    `${currentContact.firstName} ${currentContact.lastName}`;
  document.getElementById("contactDetailTitle").textContent =
    currentContact.jobTitle;

  // Set overview tab
  document.getElementById("contactDetailEmail").textContent =
    currentContact.email;
  document.getElementById("contactDetailPhone").textContent =
    currentContact.phone;
  document.getElementById("contactDetailMobile").textContent =
    currentContact.mobile || "N/A";
  document.getElementById("contactDetailLinkedin").textContent =
    currentContact.linkedin || "N/A";
  document.getElementById("contactDetailLinkedin").href =
    currentContact.linkedin ? `https://${currentContact.linkedin}` : "#";
  document.getElementById("contactDetailAddress").textContent =
    currentContact.address
      ? `${currentContact.address.street}, ${currentContact.address.city}, ${currentContact.address.postcode}`
      : "N/A";

  document.getElementById("contactDetailCompany").textContent = company
    ? company.name
    : "Unknown";
  document.getElementById("contactDetailCompany").onclick = () =>
    openCompanyProfile(currentContact.companyId);
  document.getElementById("contactDetailDepartment").textContent =
    currentContact.department || "N/A";
  ...
  tagsContainer.innerHTML = currentContact.tags
    .map(
      tag =>
        `<span class="px-2 py-0.5 bg-stone-100 text-stone-600 rounded text-xs">${tag}</span>`
    )
  ...
  document.getElementById("contactDetailPreferred").textContent =
    currentContact.preferredContact || "Email";
  document.getElementById("contactDetailLastContact").textContent = formatDate(
    currentContact.lastContact
  );
  document.getElementById("contactDetailNotes").textContent =
...
function switchContactDetailTab(tab) {
  ...
  document
    .getElementById(
      `contactDetail${tab.charAt(0).toUpperCase() + tab.slice(1)}`
    )
...
function populateContactTimeline() {
  ...
  if (!currentContact || !currentContact.timeline) {
    timelineContent.innerHTML =
      '<p class="text-stone-600">No timeline activity.</p>';
    return;
  }

  timelineContent.innerHTML = currentContact.timeline
    .map(
      (item, index) => `
...
function populateContactCommunications() {
  ...
  if (!currentContact || !currentContact.communications) {
    commsContent.innerHTML =
      '<p class="text-stone-600">No communications logged.</p>';
    return;
  }

  commsContent.innerHTML = currentContact.communications
    .map(
      comm => `
...
function populateContactProperties() {
  ...
  if (
    ...
    currentContact.properties.length === 0
  ) {
    propsContent.innerHTML =
      '<p class="text-stone-600">No associated properties.</p>';
    return;
  }

  propsContent.innerHTML = currentContact.properties
    .map(
      prop => `
    <div class="bg-stone-50 rounded-lg p-4">
...
function populateContactDocuments() {
  ...
  if (
    !currentContact ||
    !currentContact.documents ||
    currentContact.documents.length === 0
  ) {
    docsContent.innerHTML =
      '<p class="text-stone-600">No documents uploaded.</p>';
    return;
  }

  docsContent.innerHTML = currentContact.documents
    .map(
      doc => `
...
function openCompanyProfile(companyId) {
  ...
  document.getElementById("companyProfileName").textContent =
    currentCompany.name;
  document.getElementById("companyProfileIndustry").textContent =
    currentCompany.industry;

  // Set overview tab
  document.getElementById("companyLegalName").textContent =
    currentCompany.legalName;
  document.getElementById("companyRegNumber").textContent =
    currentCompany.registrationNumber;
  document.getElementById("companyVatNumber").textContent =
    currentCompany.vatNumber;
  document.getElementById("companyIndustryDetail").textContent =
    currentCompany.industry;
  document.getElementById("companySize").textContent =
    `${currentCompany.size} employees`;
  document.getElementById("companyFounded").textContent =
    currentCompany.founded;

  document.getElementById("companyAddress").textContent =
    `${currentCompany.address.street}, ${currentCompany.address.city}, ${currentCompany.address.postcode}, ${currentCompany.address.country}`;
  document.getElementById("companyWebsite").textContent =
    currentCompany.website;
  document.getElementById("companyWebsite").href =
    `https://${currentCompany.website}`;
  document.getElementById("companyLinkedin").textContent =
    currentCompany.linkedin;
  document.getElementById("companyLinkedin").href =
    `https://${currentCompany.linkedin}`;

  document.getElementById("companyPrimaryContact").textContent =
    currentCompany.primaryContact.name;
  document.getElementById("companyPrimaryEmail").textContent =
    currentCompany.primaryContact.email;
  document.getElementById("companyPrimaryPhone").textContent =
    currentCompany.primaryContact.phone;
...
function switchCompanyTab(tab) {
  ...
  document
    .getElementById(`company${tab.charAt(0).toUpperCase() + tab.slice(1)}`)
    .classList.remove("hidden");
}
...
function populateCompanyContacts() {
  ...
  if (companyContacts.length === 0) {
    contactsList.innerHTML =
      '<p class="text-stone-600">No contacts for this company.</p>';
    return;
  }

  contactsList.innerHTML = companyContacts
    .map(contact => {
      const initials = `${contact.firstName[0]}${contact.lastName[0]}`;
      const avatarColor = getAvatarColor(contact.id);
...
function populateCompanyProperties() {
  ...
  if (currentCompany.metrics.propertiesOccupied === 0) {
    propertiesList.innerHTML =
      '<p class="text-stone-600">No properties occupied by this company.</p>';
    return;
  }

  propertiesList.innerHTML =
    '<p class="text-stone-600">Property details would be populated from property data integration.</p>';
}

// Populate company metrics
function populateCompanyMetrics() {
  document.getElementById("companyPropertiesCount").textContent =
    currentCompany.metrics.propertiesOccupied;
  document.getElementById("companyUnitsCount").textContent =
    currentCompany.metrics.totalUnits;
  document.getElementById("companyAnnualRent").textContent =
    `£${(currentCompany.metrics.annualRent / 1000).toFixed(0)}k`;
  document.getElementById("companyContractValue").textContent =
    `£${(currentCompany.metrics.contractValue / 1000).toFixed(0)}k`;
  document.getElementById("companyTenantSince").textContent = currentCompany
    .metrics.tenantSince
    ? `Tenant since ${formatDate(currentCompany.metrics.tenantSince)}`
    : "Prospect - not yet a tenant";
}
