...
const topProperties = [
  { id: 1, name: "Canary Wharf Tower", occupancy: 98, revenue: 450000, badge: "🔥" },
  { id: 2, name: "Shoreditch Hub", occupancy: 95, revenue: 380000, badge: "⭐" },
  { id: 3, name: "King's Cross Central", occupancy: 92, revenue: 340000, badge: "📈" },
  { id: 4, name: "Liverpool Street", occupancy: 88, revenue: 295000, badge: "⭐" },
  { id: 5, name: "Old Street", occupancy: 85, revenue: 250000, badge: "📈" }
];

const propertyAlerts = [
  { id: 1, type: "warning", property: "Bishopsgate Plaza", message: "Low occupancy (68%)", severity: "medium" },
  { id: 2, type: "info", property: "The Shard Suite", message: "Lease expiring in 45 days", severity: "low" },
  { id: 3, type: "success", property: "Tech Hub London", message: "High performer - upsell opportunity", severity: "low" }
];
...
const brokerData = [
  { id: 1, name: "David Brown", referrals: 3, score: 95, revenue: 320000, conversion: 73 },
  { id: 2, name: "Sarah Johnson", referrals: 2, score: 88, revenue: 265000, conversion: 67 },
  { id: 3, name: "Mike Rodriguez", referrals: 1, score: 82, revenue: 198000, conversion: 60 }
];

const commissionAlerts = [
  { id: 1, broker: "David Brown", deal: "Canary Wharf deal", amount: 18500, status: "pending" },
  { id: 2, broker: "Sarah Johnson", deal: "Shoreditch lease", amount: 12750, status: "pending" },
  { id: 3, broker: "Mike Rodriguez", deal: "King's Cross", amount: 14000, status: "pending" }
];
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-2xl font-bold">{portfolioData.totalProperties}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500">Occupied</p>
                    <p className="text-2xl font-bold text-green-600">{portfolioData.occupiedUnits}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500">Available</p>
                    <p className="text-2xl font-bold text-blue-600">{portfolioData.availableUnits}</p>
                  </div>
                  <div>
                ...
                <div className="pt-4 border-t">
                  ...
                  <Progress value={portfolioData.occupancyRate} className="h-2" />
                </div>
              </CardContent>
            </Card>
                  ...
                  {propertyAlerts.map((alert) => (
                    ...
                    >
                      <div className="flex items-start gap-2">
                        {alert.severity === 'high' && <AlertCircle className="h-4 w-4 text-red-600 mt-0.5" />}
                        {alert.severity === 'medium' && <Clock className="h-4 w-4 text-amber-600 mt-0.5" />}
                        {alert.severity === 'low' && <CheckCircle2 className="h-4 w-4 text-blue-600 mt-0.5" />}
                        <div className="flex-1">
                          <p className="font-medium text-sm">{alert.property}</p>
                ...
                <div className="space-y-4">
                  ...
                  <div className="relative">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      <div className="flex-1 h-14 bg-blue-500 rounded flex items-center justify-center text-white font-semibold" style={{ width: '78%' }}>
                        {pipelineData.qualified} Qualified
                      </div>
                      <span className="text-sm text-gray-500">78%</span>
                    </div>
                    <div className="flex items-center gap-2 mt-2">
                      <div className="flex-1 h-12 bg-blue-400 rounded flex items-center justify-center text-white font-semibold" style={{ width: '45%' }}>
                        {pipelineData.matched} Matched
                      </div>
                      <span className="text-sm text-gray-500">58%</span>
                    </div>
                    <div className="flex items-center gap-2 mt-2">
                      <div className="flex-1 h-10 bg-blue-300 rounded flex items-center justify-center text-white font-semibold" style={{ width: '32%' }}>
                        {pipelineData.viewing} Viewing
                      </div>
                      <span className="text-sm text-gray-500">71%</span>
                    </div>
                    <div className="flex items-center gap-2 mt-2">
                      <div className="flex-1 h-8 bg-green-500 rounded flex items-center justify-center text-white font-semibold" style={{ width: '18%' }}>
                      ...
                      >
                        <div className="flex items-start justify-between mb-3">
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <Badge 
                                variant={lead.grade === 'A' ? 'default' : 'secondary'}
                                className={
                        ...
                        <div className="grid grid-cols-3 gap-4 mb-3">
                          <div>
                            ...
                            <div className="flex items-center gap-1">
                              <p className="text-lg font-bold">{lead.bantScore}</p>
                              <Progress value={lead.bantScore} className="h-1 flex-1" />
                            </div>
                          </div>
                          <div>
                            <p className="text-xs text-gray-500">Budget</p>
                            <p className="text-sm font-semibold">{lead.budget}</p>
                          </div>
                          <div>
                            <p className="text-xs text-gray-500">Timeline</p>
                            <p className="text-sm font-semibold">{lead.timeline}</p>
                          </div>
                        </div>
                        
                        <div className="flex items-center gap-2">
                          <Button size="sm" className="flex-1">
                            {lead.status === 'new' ? 'Qualify Now' : 'View Details'}
                          </Button>
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent>
                              <DropdownMenuItem>Match Properties</DropdownMenuItem>
                              <DropdownMenuItem>Schedule Viewing</DropdownMenuItem>
                              <DropdownMenuItem>Assign to Max</DropdownMenuItem>
                              <DropdownMenuSeparator />
                              <DropdownMenuItem className="text-red-600">Disqualify</DropdownMenuItem>
                            </DropdownMenuContent>
                          </DropdownMenu>
                        </div>
                      </div>
                    ...
                    <div key={source.source} className="space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-4 text-sm">
                          ...
                          <span className="text-green-600 font-semibold">{source.conversion}% conv.</span>
                        </div>
                      </div>
                  ...
                  {brokerData.map((broker, index) => (
                    ...
                    >
                      <div className="flex items-start justify-between mb-2">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-500">{broker.referrals} referrals this week</p>
                          </div>
                        </div>
                        <Badge variant="outline" className="text-green-600 border-green-600">
                          {broker.score}
                      ...
                      <div className="flex items-center justify-between text-xs mt-2">
                        <span className="text-gray-600">{broker.conversion}% conversion</span>
                        <span className="font-semibold">£{(broker.revenue / 1000).toFixed(0)}K</span>
                      </div>
                    ...
                    <div className="space-y-3">
                      {commissionAlerts.map((commission) => (
                        ...
                        >
                          <div className="flex items-start justify-between mb-2">
                            <div>
                              <p className="font-semibold text-sm">{commission.broker}</p>
                              <p className="text-xs text-gray-600">{commission.deal}</p>
                            </div>
