...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", function () {
    ...
    if (propertyIndexPage) {
      propertyIndexObserver.observe(propertyIndexPage, {
        attributes: true,
...
function initializePropertyIndex() {
  ...
  if (typeof propertiesData === "undefined" || !propertiesData) {
    console.error(
      "propertiesData is not defined! Cannot initialize Property Index."
    );
    console.log(
      "Available global variables:",
      Object.keys(window).filter(k => k.toLowerCase().includes("prop"))
    );
  ...
  console.log(
    "Initializing Property Index with",
...
function applyFiltersAndSort() {
  ...
  if (currentFilters.location) {
    filtered = filtered.filter(
      property => property.location === currentFilters.location
    );
  }
  ...
  if (currentFilters.type) {
    filtered = filtered.filter(
      property => property.type === currentFilters.type
    );
  }
  ...
  if (currentFilters.availability) {
    filtered = filtered.filter(
      property => property.status === currentFilters.availability
    );
  }
  ...
  if (currentFilters.size) {
    filtered = filtered.filter(property => {
      ...
      if (currentFilters.size === "medium")
        return size >= 5000 && size <= 20000;
...
function updatePropertyCount() {
  ...
  const showingBottomElement = document.getElementById(
    "properties-showing-bottom"
  );
...
function renderGridView() {
  ...
  gridContainer.innerHTML = filteredProperties
    .map(property => {
      ...
      occupancyBgClass =
        property.occupancyRate >= 85 ? "bg-green-500" : "bg-amber-500";
...
function renderListView() {
  ...
  listContainer.innerHTML = filteredProperties
    .map(property => {
      // Map status to Tailwind color classes
      let statusBgClass, statusTextClass, occupancyBgClass;
...
function debugInitialization() {
  ...
  console.log(
    ...
    !document
      .getElementById("property-index-page")
  ...
  console.log(
    "propertiesData length:",
    typeof propertiesData !== "undefined" ? propertiesData.length : "N/A"
  );
