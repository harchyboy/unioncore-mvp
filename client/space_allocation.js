...
function calculatePortfolioMetrics() {
  ...
  const totalSpace = propertiesData.reduce(
    (sum, prop) => sum + prop.totalArea,
    0
  );
  const totalUnits = propertiesData.reduce(
    (sum, prop) => sum + prop.totalUnits,
    0
  );
  const occupiedUnits = propertiesData.reduce(
  ...
  document.getElementById("occupied-space").textContent =
    Math.round(occupiedSpace).toLocaleString();
  document.getElementById("occupancy-percentage").textContent =
    occupancyPercentage;
  document.getElementById("available-space").textContent =
    Math.round(availableSpace).toLocaleString();
  ...
  if (selectedProperty === "all") {
    ...
    if (property) {
      ...
      for (let floor = property.floors; floor >= 1; floor--) {
        const floorUnits = property.units.filter(u =>
          u.floor.includes(floor.toString())
        );
        const occupiedUnits = floorUnits.filter(
          u => u.status === "Occupied"
        ).length;
        const totalFloorUnits = floorUnits.length || 1;
        const occupancyRate = ((occupiedUnits / totalFloorUnits) * 100).toFixed(
          0
        );
...
function populateCapacityChart() {
  ...
  const sortedProperties = [...propertiesData].sort(
...
function populateTenantTypeChart() {
  ...
  propertiesData.forEach(prop => {
    prop.units.forEach(unit => {
      if (unit.tenant && unit.tenant !== "-") {
        ...
        if (
          tenant.includes("tech") ||
          tenant.includes("digital") ||
          tenant.includes("software")
        ) {
          tenantTypes["Tech"] += unit.size;
        } else if (
          tenant.includes("capital") ||
          tenant.includes("finance") ||
          tenant.includes("investment")
        ) {
          tenantTypes["Finance"] += unit.size;
        } else if (
          tenant.includes("consulting") ||
          tenant.includes("law") ||
          tenant.includes("advisory")
        ) {
          tenantTypes["Professional Services"] += unit.size;
        } else if (
          tenant.includes("media") ||
          tenant.includes("design") ||
          tenant.includes("creative")
        ) {
  ...
  container.innerHTML = `
        <div class="space-y-3">
            ${Object.entries(tenantTypes)
              .map(([type, space], index) => {
                const percentage =
                  total > 0 ? ((space / total) * 100).toFixed(1) : 0;
                return `
                    <div>
                        <div class="flex items-center justify-between mb-1">
...
function populateSharedSpaceBooking() {
  ...
  const sampleBookings = [
    ...
    },
    {
      ...
      property: "Shoreditch Studios",
      date: "Tomorrow, 10:00 AM",
      duration: "1 hour",
    },
    {
      space: "Event Space",
      property: "King's Cross Hub",
      date: "Friday, 3:00 PM",
      duration: "3 hours",
    },
  ];

  bookingsContainer.innerHTML = sampleBookings
    .map(
      booking => `
...
function generateSharedCalendar() {
  ...
  const firstDay = new Date(
    sharedCalendarDate.getFullYear(),
    sharedCalendarDate.getMonth(),
    1
  );
  const lastDay = new Date(
    sharedCalendarDate.getFullYear(),
    sharedCalendarDate.getMonth() + 1,
    0
  );
...
function populateFuturePlanning() {
  ...
  const pipelinePercentage = (
    (pipelineSpaceNeeded / availableSpace) *
    100
  ).toFixed(1);

  document.getElementById("pipeline-space-needed").textContent =
    pipelineSpaceNeeded.toLocaleString();
  document.getElementById("pipeline-deals-count").textContent = pipelineDeals;
  document.getElementById("pipeline-capacity-bar").style.width =
    Math.min(pipelinePercentage, 100) + "%";
  ...
  forecastContainer.innerHTML = months
    .map((month, index) => {
      const projectedOccupancy = Math.min(
        parseFloat(currentOccupancy) + index * 2,
        95
      );
  ...
  if (pipelinePercentage > 80) {
    recommendations.push(
      "Consider acquiring additional properties to meet pipeline demand"
    );
  }
  ...
  if (lowOccupancyProps.length > 0) {
    recommendations.push(
      `Focus marketing efforts on ${lowOccupancyProps.length} properties with low occupancy`
    );
  }
  ...
  if (highOccupancyProps.length > 0) {
    recommendations.push(
      `${highOccupancyProps.length} properties near capacity - prepare expansion plans`
    );
  }
...
function populateSpaceBreakdownTable() {
  ...
  tableBody.innerHTML = propertiesData
    .map(prop => {
      const occupiedSpace = Math.round(
        prop.totalArea * (prop.occupancyRate / 100)
      );
      const availableSpace = prop.totalArea - occupiedSpace;
      const tenantCount = prop.units.filter(
        u => u.tenant && u.tenant !== "-"
      ).length;
